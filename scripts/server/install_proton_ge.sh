...
set -euo pipefail
  ...
  is_proton_ge_installed(){
      log "LOG" "VÉRIFICATION & INSTALLATION DE PROTON GE $PROTON_GE_VERSION SUR LE SERVEUR $ARK_SERVER_SERVICE."
  ...
  move_to_proton_ge_temp_dir(){
    ...
    if cd "$PROTON_GE_TEMP_DIR"; then
      ...
      log "DEBUG" "VEUILLEZ ESSAYER DE VOUS DÉPLACER MANUELLEMENT DANS LE RÉPERTOIRE TEMPORAIRE A L'AIDE DE LA COMMANDE SUIVANTE:"
  ...
  download_proton_ge(){
    ...
    if sudo curl -# -L "$PROTON_GE_TGZ" -o "$PROTON_GE_VERSION.tar.gz"; then
      log "SUCCESS" "L'ARCHIVE DE PROTON $PROTON_GE_VERSION A ÉTÉ TÉLÉCHARGÉE DANS LE RÉPERTOIRE TEMPORAIRE."
    else
  ...
  download_proton_ge_checksum(){
    ...
    if sudo curl -# -L "$PROTON_GE_CHECKSUM" -o "$PROTON_GE_SHA"; then
      ...
      log "DEBUG" "VEUILLEZ ESSAYER DE TÉLÉCHARGER LA SOMME DE CONTRÔLE SHA512 MANUELLEMENT A L'AIDE DE LA COMMANDE SUIVANTE:"
